...
# 2*np.pi*299792458/omega_p, where omega_p = 1.3688599705386422e+16 (rad)
...
for it, geom_tSlab in zip(range(tSlabArray.size), tSlabArray):
        ...
        if (model.w==0.37):
            ...
            for i in range(0, 3):
                ...
                if np.abs(n2c.real)<1e-12:
                    axs[i+ioffset].axline((0, 0), slope=np.divide(-n2c.real, n2c.imag), linestyle="--", linewidth=1, c="k")
                else:
                    axs[i+ioffset].axline((0, 0), (-n2c.imag/n2c.real, 1), linestyle="--", linewidth=1, c="k")
    ...
    for i in range(0, 4):
        ...
        if i>=2:
            axs[i+1].text(0.5, -0.35, "x (nm)", usetex=True, transform=axs[i+1].transAxes, fontsize=fontsize*1.25, ha="center", va="top")
