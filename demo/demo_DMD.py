...
geom_hMax = model.ld0_target/21; geom_nPML = 11; # TBC debugging
...
for it, geom_tSlab in zip(range(tSlabArray.size), tSlabArray):
    ...
    # for iw in range(model.wArray.size)[::-1]: # in reverse order
        ...
        if (model.w==0.6):
            ...
            zfx = np.array([2.5, 0.15])
            zfy = np.array([0.15, 2.5])
            for i in range(0, 2):
                axs[i+3].set_xlim([-kappa_max*zfx[i], kappa_max*zfx[i]])
                axs[i+3].set_ylim([-kappa_max*zfy[i], kappa_max*zfy[i]])
    ...
    ax_hz1.text(0.15, 0.15, "$\\mathrm{TM_0}$", usetex=True, transform=ax_hz1.transAxes, fontsize=fontsize, ha="center", va="center", fontweight="bold", color="C0")

    ax_hz2.text(0.15, 0.15, "$\\mathrm{TM_1}$", usetex=True, transform=ax_hz2.transAxes, fontsize=fontsize, ha="center", va="center", fontweight="bold", color="C1")
    ...
    for iSx in [+1, -1]:
        for itau_im in [+1, -1]:
            ...
            ax_tau.text(0.35*kappa_max*zfx[1]*iSx, 0.25*kappa_max*zfy[1]*itau_im,
